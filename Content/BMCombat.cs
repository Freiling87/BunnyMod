...
namespace BunnyMod.Content
{
	public class BMCombat
	{
		...
		public static bool MeleeHitbox_HitObject(GameObject hitObject, bool fromClient, MeleeHitbox __instance) // Replacement
		{
			...
			{
				if (GC.multiplayerMode && GC.serverPlayer && !__instance.myMelee.agent.localPlayer && !fromClient && __instance.myMelee.agent.isPlayer != 0)
				{
					bool flag = true;
					...
					else if (hitObject.CompareTag("AgentSprite"))
					{
						...
						if (agent == __instance.myMelee.agent)
						{
							flag = false;
						}
						if (agent.localPlayer && __instance.myMelee.agent.isPlayer > 0 && !__instance.myMelee.agent.localPlayer)
						{
							flag = false;
						}
					}
				...
				{
					if (__instance.myMelee.agent.statusEffects.hasTrait("BlocksSometimesHit2"))
					{
					...
					else if (__instance.myMelee.agent.statusEffects.hasTrait("BlocksSometimesHit"))
					{
					...
					else
					{
						...
						if (agent2.statusEffects.hasTrait("BlocksSometimesHit2"))
						{
							if (GC.percentChance(60))
							{
								hitObject = __instance.myMelee.agent.agentHitboxScript.go;
							}
						}
				...
				if (hitObject.CompareTag("ObjectRealSprite") && !__instance.myMelee.invItem.meleeNoHit)
				{
					...
					if (!GC.serverPlayer && !__instance.myMelee.agent.localPlayer && __instance.myMelee.agent.mindControlAgent != GC.playerAgent && objectReal2.objectSprite.meshRenderer.enabled && !objectReal2.notRealObject && !objectReal2.OnFloor && !objectReal2.meleeCanPass && !objectReal2.tempNoMeleeHits)
					{
						__instance.FakeHit(hitObject);
						return false;
					}
					...
					{
						bool flag2 = false;
						if (__instance.myMelee.agent.statusEffects.hasTrait("HitObjectsNoNoise"))
						{
							flag2 = true;
						}
						...
						if (!objectReal2.notRealObject && !objectReal2.OnFloor && (!objectReal2.meleeCanPass || __instance.justHitWindow) && !objectReal2.tempNoMeleeHits)
						{
							...
							{
								GC.spawnerMain.SpawnNoise(objectReal2.FindDamageNoisePos(objectReal2.tr.position), (float)objectReal2.noiseHitVol, null, null, __instance.myMelee.agent);
							}
							if (__instance.myMelee.agent != null && GC.serverPlayer)
							{
								GC.OwnCheck(__instance.myMelee.agent, hitObject, "Normal", 1);
							}
							...
							{
								if (__instance.myMelee.agent.isPlayer > 0 && __instance.myMelee.agent.localPlayer && !objectReal2.noDestroyEffects)
								{
									GC.ScreenShake(0.2f, 80f, __instance.myMelee.agent.tr.position, __instance.myMelee.agent);
									GC.FreezeFrames(1);
								}
							}
							else if (__instance.myMelee.agent.isPlayer > 0 && __instance.myMelee.agent.localPlayer)
							{
								GC.ScreenShake(0.1f, 80f, __instance.myMelee.agent.tr.position, __instance.myMelee.agent);
								GC.FreezeFrames(0);
							}
							GC.alienFX.HitObject(__instance.myMelee.agent);
							__instance.MeleeHitEffect(hitObject);
							GC.playerControl.Vibrate(__instance.myMelee.agent.isPlayer, Mathf.Clamp((float)objectReal2.damagedAmount / 100f + 0.05f, 0f, 0.25f), Mathf.Clamp((float)objectReal2.damagedAmount / 132f + 0.05f, 0f, 0.2f));
							if (!GC.serverPlayer && (__instance.myMelee.agent.isPlayer > 0 || __instance.myMelee.agent.mindControlAgent == GC.playerAgent))
							{
								...
								{
									__instance.myMelee.agent.objectMult.CallCmdMeleeHitObjectReal(objectReal2.objectNetID);
									return false;
								}
								GC.playerAgent.objectMult.CallCmdMeleeHitObjectRealNPC(__instance.myMelee.agent.objectNetID, objectReal2.objectNetID);
								return false;
							}
							else if (GC.serverPlayer && GC.multiplayerMode)
							{
								__instance.myMelee.agent.objectMult.CallRpcMeleeHitObjectFake(objectReal2.objectNetID);
								return false;
							}
						}
					}
				...
				else if (hitObject.CompareTag("AgentSprite"))
				{
					...
					{
						if (GC.serverPlayer && __instance.myMelee.agent.localPlayer && hitAgent.isPlayer > 0 && !hitAgent.localPlayer)
						{
							__instance.FakeHit(hitObject);
							return false;
						}
						if (GC.serverPlayer && __instance.myMelee.agent.isPlayer == 0 && hitAgent.isPlayer > 0 && !hitAgent.localPlayer)
						{
							__instance.FakeHit(hitObject);
							return false;
						}
						if (GC.multiplayerMode && GC.serverPlayer && __instance.myMelee.agent.isPlayer == 0 && hitAgent.isPlayer == 0 && __instance.myMelee.agent.mindControlAgent != null && __instance.myMelee.agent.mindControlAgent != GC.playerAgent && !hitAgent.dead)
						{
							__instance.FakeHit(hitObject);
							return false;
						}
						if (GC.multiplayerMode && GC.serverPlayer && __instance.myMelee.agent.isPlayer == 0 && hitAgent.isPlayer == 0 && hitAgent.mindControlAgent != null && hitAgent.mindControlAgent != GC.playerAgent && !hitAgent.dead)
						{
							__instance.FakeHit(hitObject);
							return false;
						}
						if (!GC.serverPlayer && __instance.myMelee.agent.isPlayer == 0 && !hitAgent.localPlayer && __instance.myMelee.agent != hitAgent && ((__instance.myMelee.agent.mindControlAgent != GC.playerAgent && hitAgent.mindControlAgent != GC.playerAgent) || hitAgent.dead))
						{
							__instance.FakeHit(hitObject);
							return false;
						}
						if (!GC.serverPlayer && __instance.myMelee.agent.isPlayer > 0 && !__instance.myMelee.agent.localPlayer && !hitAgent.localPlayer)
						{
							__instance.FakeHit(hitObject);
							return false;
						}
						if (!GC.serverPlayer && (__instance.myMelee.agent.localPlayer || __instance.myMelee.agent.mindControlAgent == GC.playerAgent) && hitAgent.isPlayer > 0 && !hitAgent.localPlayer)
						{
							__instance.FakeHit(hitObject);
							return false;
						}
						if (!GC.serverPlayer && __instance.myMelee.agent.isPlayer != 0 && !__instance.myMelee.agent.localPlayer && hitAgent.isPlayer != 0 && !hitAgent.localPlayer)
						{
							__instance.FakeHit(hitObject);
							return false;
						}
					}
					if (__instance.myMelee.agent != hitAgent && 
						!(hitAgent.ghost && !__instance.myMelee.agent.statusEffects.hasTrait(cTrait.SpectralStrikes) && !__instance.myMelee.agent.statusEffects.hasTrait(cTrait.SpectralStrikes_2)) 
						&& !hitAgent.fellInHole && !GC.cinematic && __instance.HasLOSMelee(hitAgent))
					{
						...
						{
							Relationship relationship = hitAgent.relationships.GetRelationship(__instance.myMelee.agent);
							if (!hitAgent.movement.HasLOSObjectBehind(__instance.myMelee.agent) || hitAgent.sleeping || __instance.myMelee.agent.isPlayer == 0 || __instance.myMelee.agent.invisible || (__instance.myMelee.invItem.invItemName == "StealingGlove" && __instance.myMelee.agent.oma.superSpecialAbility))
							{
								...
								{
									if (__instance.myMelee.agent.oma.superSpecialAbility)
									{
										__instance.HitAftermath(hitObject, fromClient, hitAgent, __instance.myMelee.invItem.invItemName);
									}
									else if (__instance.myMelee.agent.movement.HasLOSObject(hitAgent, "360", false, true))
									{
							...
							{
								if (__instance.myMelee.invItem.invItemName == "StealingGlove" && relationship.relType != "Aligned" && relationship.relType != "Submissive" && __instance.myMelee.agent.movement.HasLOSObject(hitAgent, "360", false, true) && !fromClient)
								{
						...
						bool flag3 = !__instance.myMelee.invItem.meleeNoHit && __instance.myMelee.agent.DontHitAlignedCheck(hitAgent);
						if (flag3)
						{
							hitAgent.melee.meleeHitbox.objectList.Add(__instance.gameObject);
							hitAgent.melee.meleeHitbox.objectList.Add(__instance.myMelee.agent.sprTr.gameObject);
							if (__instance.myMelee.agent.zombified && hitAgent.isPlayer == 0 && !hitAgent.oma.bodyGuarded)
							{
								hitAgent.zombieWhenDead = true;
							}
							if (hitAgent.isPlayer == 0 && __instance.myMelee.agent.isPlayer != 0 && !hitAgent.dead && hitAgent.agentName != "Zombie" && !hitAgent.inhuman && !hitAgent.mechEmpty && !hitAgent.mechFilled && __instance.myMelee.agent.localPlayer && !hitAgent.statusEffects.hasStatusEffect("Invincible"))
							{
							...
							__instance.myMelee.agent.relationships.FollowerAlert(hitAgent);
							if (hitAgent.statusEffects.hasTrait("AttacksDamageAttacker2") && !(__instance.myMelee.agent.ghost && !__instance.myMelee.agent.statusEffects.hasTrait(cTrait.SpectralStrikes) && !__instance.myMelee.agent.statusEffects.hasTrait(cTrait.SpectralStrikes_2)))
							{
								int myChance = hitAgent.DetermineLuck(20, "AttacksDamageAttacker", true);
								if (GC.percentChance(myChance))
								{
									__instance.myMelee.agent.lastHitByAgent = hitAgent;
									__instance.myMelee.agent.justHitByAgent2 = hitAgent;
									__instance.myMelee.agent.lastHitByAgent = hitAgent;
									__instance.myMelee.agent.deathMethod = "AttacksDamageAttacker";
									__instance.myMelee.agent.deathKiller = hitAgent.agentName;
									__instance.myMelee.agent.statusEffects.ChangeHealth(-10f);
								}
							}
							else if (hitAgent.statusEffects.hasTrait("AttacksDamageAttacker") && !__instance.myMelee.agent.ghost)
							{
							...
							if (hitAgent.justDied && __instance.myMelee.agent.isPlayer > 0 && !GC.coopMode && !GC.fourPlayerMode && !GC.multiplayerMode && GC.sessionDataBig.slowMotionCinematic && GC.percentChance(25))
							{
							...
							{
								hitAgent.movement.KnockBackBullet(__instance.myMelee.meleeContainerTr.gameObject, num, true, __instance.myMelee.agent);
							}
							bool flag4 = false;
							...
							if (flag4)
							{
								...
								{
									if (hitAgent.isPlayer != 0 && !hitAgent.localPlayer && __instance.myMelee.agent.isPlayer == 0)
									{
										flag5 = false;
									}
									if (__instance.myMelee.agent.isPlayer != 0 && !__instance.myMelee.agent.localPlayer && hitAgent.isPlayer == 0)
									{
										flag5 = false;
									}
								}
								if (flag5)
								{
									__instance.myMelee.agent.movement.KnockBackBullet(hitAgent.gameObject, 240f, true, hitAgent);
									if (GC.serverPlayer && __instance.myMelee.agent.isPlayer == 0 && invItem.invItemName != "Fist" && !hitAgent.warZoneAgent)
									{
							...
							{
								__instance.myMelee.agent.objectMultPlayfield.TempDisableNetworkTransform(hitAgent);
								...
								if (!__instance.myMelee.agent.testingNewClientLerps)
								{
									if (__instance.myMelee.agent.isPlayer != 0)
									{
										__instance.myMelee.agent.objectMult.CallCmdMeleeHitAgent(hitAgent.objectNetID, position2, (int)num, position, hitAgent.rb.velocity);
									}
									else
									{
										GC.playerAgent.objectMult.CallCmdMeleeHitAgentNPC(__instance.myMelee.agent.objectNetID, hitAgent.objectNetID, position2, (int)num, position, hitAgent.rb.velocity);
									}
								}
							}
							else if (GC.multiplayerMode && GC.serverPlayer)
							{
								__instance.myMelee.agent.objectMult.CallRpcMeleeHitObjectFake(hitAgent.objectNetID);
							}
							if ((__instance.myMelee.agent.isPlayer > 0 && __instance.myMelee.agent.localPlayer) || (hitAgent.isPlayer > 0 && hitAgent.localPlayer))
							{
								...
								{
									GC.ScreenShake(0.25f, (float)Mathf.Clamp(15 * hitAgent.damagedAmount, 160, 500), Vector2.zero, __instance.myMelee.agent);
								}
								else
								{
									GC.ScreenShake(0.2f, (float)Mathf.Clamp(15 * hitAgent.damagedAmount, 0, 500), Vector2.zero, __instance.myMelee.agent);
								}
							}
							...
							__instance.MeleeHitEffect(hitObject);
							GC.playerControl.Vibrate(__instance.myMelee.agent.isPlayer, Mathf.Clamp((float)hitAgent.damagedAmount / 100f + 0.05f, 0f, 0.25f), Mathf.Clamp((float)hitAgent.damagedAmount / 132f + 0.05f, 0f, 0.2f));
				...
				else if (hitObject.CompareTag("ItemImage"))
				{
					...
					if (!component2.justSpilled && !__instance.myMelee.invItem.meleeNoHit && component2.itemObject == null && __instance.HasLOSMelee(component2))
					{
						...
						if (!fromClient)
						{
							component2.movement.KnockBackBullet(__instance.myMelee.meleeContainerTr.gameObject, 250f, true, __instance.myMelee.agent);
						}
						component2.Damage(__instance.myMelee, fromClient);
						if (component2.invItem.reactOnTouch)
						{
							component2.TouchMe(__instance.myMelee.agent, "MeleeHitbox");
						}
						component2.thrower = __instance.myMelee.agent;
						if (component2.go.activeSelf)
						{
							component2.StartCoroutine(component2.HitCauserCoroutine(__instance.myMelee.agent));
						}
						if (GC.serverPlayer && !__instance.myMelee.successfullySleepKilled && !__instance.myMelee.agent.statusEffects.hasTrait("HitObjectsNoNoise"))
						{
							GC.spawnerMain.SpawnNoise(__instance.tr.position, 1f, null, null, __instance.myMelee.agent);
						}
						if (component2.startingOwner != 0 && GC.serverPlayer && !__instance.myMelee.agent.statusEffects.hasTrait("NoStealPenalty"))
						{
							GC.OwnCheck(__instance.myMelee.agent, hitObject.transform.parent.gameObject, "Normal", 1);
						}
						Physics2D.IgnoreCollision(component2.myCollider2D, __instance.myMelee.agent.myCollider2D, true);
						Physics2D.IgnoreCollision(component2.myCollider2D, __instance.myMelee.agent.agentItemCollider, true);
						__instance.MeleeHitEffect(hitObject);
						GC.playerControl.Vibrate(__instance.myMelee.agent.isPlayer, Mathf.Clamp((float)__instance.myMelee.invItem.meleeDamage / 100f + 0.05f, 0f, 0.25f), Mathf.Clamp((float)__instance.myMelee.invItem.meleeDamage / 132f + 0.05f, 0f, 0.2f));
						if (!GC.serverPlayer && (__instance.myMelee.agent.localPlayer || __instance.myMelee.agent.mindControlAgent == GC.playerAgent))
						{
							__instance.myMelee.agent.objectMultPlayfield.TempDisableNetworkTransform(component2);
							...
							if (__instance.myMelee.agent.isPlayer != 0)
							{
								__instance.myMelee.agent.objectMult.CallCmdMeleeHitItem(component2.objectNetID, position4, 250, position3, component2.rb.velocity);
								return false;
							}
							GC.playerAgent.objectMult.CallCmdMeleeHitItemNPC(__instance.myMelee.agent.objectNetID, component2.objectNetID, position4, 250, position3, component2.rb.velocity);
							return false;
						}
				...
				else if (hitObject.CompareTag("MeleeHitbox"))
				{
					...
					if (GC.serverPlayer && __instance.myMelee.agent.isPlayer == 0 && agent4.isPlayer > 0 && !agent4.localPlayer)
					{
						return false;
					}
					if (!GC.serverPlayer && __instance.myMelee.agent.isPlayer == 0 && !agent4.localPlayer && __instance.myMelee.agent != agent4 && ((agent4.mindControlAgent != GC.playerAgent && __instance.myMelee.agent.mindControlAgent != GC.playerAgent) || agent4.dead))
					{
						return false;
					}
					if (GC.multiplayerMode && GC.serverPlayer && __instance.myMelee.agent.isPlayer == 0 && agent4.isPlayer == 0 && __instance.myMelee.agent.mindControlAgent != null && __instance.myMelee.agent.mindControlAgent != GC.playerAgent && !agent4.dead)
					{
						__instance.FakeHit(hitObject);
						return false;
					}
					if (GC.multiplayerMode && GC.serverPlayer && __instance.myMelee.agent.isPlayer == 0 && agent4.isPlayer == 0 && agent4.mindControlAgent != null && agent4.mindControlAgent != GC.playerAgent && !agent4.dead)
					{
						__instance.FakeHit(hitObject);
						return false;
					}
					if (!GC.serverPlayer && __instance.myMelee.agent.isPlayer > 0 && !__instance.myMelee.agent.localPlayer && !agent4.localPlayer)
					{
						return false;
					}
					if (!__instance.myMelee.agent.DontHitAlignedCheck(agent4))
					{
						return false;
					}
					if (__instance.myMelee.agent != agent4 && __instance.myMelee.agent.agentSpriteTransform.localScale.x != 3f && agent4.agentSpriteTransform.localScale.x != 3f && __instance.myMelee.agent.agentSpriteTransform.localScale.x > 0.34f && agent4.agentSpriteTransform.localScale.x > 0.34f && !__instance.myMelee.invItem.meleeNoHit && __instance.HasLOSMelee(melee.agent))
					{
						melee.meleeHitbox.objectList.Add(__instance.gameObject);
						__instance.objectList.Add(agent4.sprTr.gameObject);
						melee.meleeHitbox.objectList.Add(__instance.myMelee.agent.sprTr.gameObject);
						int num2 = __instance.myMelee.agent.FindDamageTestOnly(hitObject.GetComponent<MeleeColliderBox>().meleeHitbox.myMelee);
						...
						if (GC.multiplayerMode && GC.serverPlayer)
						{
							if (agent4.isPlayer != 0 && !agent4.localPlayer && __instance.myMelee.agent.isPlayer == 0)
							{
								flag6 = false;
							}
							if (__instance.myMelee.agent.isPlayer != 0 && !__instance.myMelee.agent.localPlayer && agent4.isPlayer == 0)
							{
								flag6 = false;
							}
						}
						if (flag6)
						{
							__instance.myMelee.agent.movement.KnockBackBullet(melee.meleeContainerTr.gameObject, (float)num5, true, melee.agent);
							if (!fromClient)
							{
								agent4.movement.KnockBackBullet(__instance.myMelee.meleeContainerTr.gameObject, (float)num4, true, __instance.myMelee.agent);
							}
						}
						if (!GC.serverPlayer)
						{
							Agent agent5 = null;
							Agent agent6 = null;
							if (__instance.myMelee.agent.localPlayer || __instance.myMelee.agent.mindControlAgent == GC.playerAgent)
							{
								agent5 = __instance.myMelee.agent;
								agent6 = agent4;
							}
							else if (agent4.localPlayer || agent4.mindControlAgent == GC.playerAgent)
							{
								agent5 = agent4;
								agent6 = __instance.myMelee.agent;
							}
							if (agent5 != null)
							{
						...
						{
							__instance.myMelee.agent.objectMult.CallRpcMeleeHitObjectFake(agent4.objectNetID);
						}
						if (!__instance.myMelee.agent.ghost && 
							!(agent4.ghost && !__instance.myMelee.agent.statusEffects.hasTrait(cTrait.SpectralStrikes) && !__instance.myMelee.agent.statusEffects.hasTrait(cTrait.SpectralStrikes_2)))
						{
							if (invItem.invItemName != "Fist" && agent4.inventory.equippedWeapon.invItemName == "Fist")
							{
								agent4.lastHitByAgent = __instance.myMelee.agent;
								agent4.justHitByAgent2 = __instance.myMelee.agent;
								agent4.lastHitByAgent = __instance.myMelee.agent;
								...
								agent4.deathKiller = __instance.myMelee.agent.agentName;
								if (!GC.serverPlayer || __instance.myMelee.agent.localPlayer || __instance.myMelee.agent.isPlayer == 0)
								{
									if (__instance.myMelee.agent.zombified && agent4.isPlayer == 0 && !agent4.oma.bodyGuarded)
									{
							...
							{
								__instance.myMelee.agent.lastHitByAgent = agent4;
								__instance.myMelee.agent.justHitByAgent2 = agent4;
								__instance.myMelee.agent.lastHitByAgent = agent4;
								agent4.deathMethodItem = invItem.invItemName;
								agent4.deathMethodObject = invItem.invItemName;
								__instance.myMelee.agent.deathMethod = agent4.inventory.equippedWeapon.invItemName;
								__instance.myMelee.agent.deathKiller = agent4.agentName;
								if (!GC.serverPlayer || __instance.myMelee.agent.localPlayer || __instance.myMelee.agent.isPlayer == 0)
								{
									if (agent4.zombified && __instance.myMelee.agent.isPlayer == 0 && !__instance.myMelee.agent.oma.bodyGuarded)
									{
										__instance.myMelee.agent.zombieWhenDead = true;
									}
									__instance.myMelee.agent.statusEffects.ChangeHealth(-1f);
								}
							}
						}
						__instance.myMelee.agent.inventory.DepleteMelee(5);
						agent4.inventory.DepleteMelee(5);
						__instance.MeleeHitEffect(hitObject);
						if ((__instance.myMelee.agent.isPlayer > 0 && __instance.myMelee.agent.localPlayer) || (melee.agent.isPlayer > 0 && melee.agent.localPlayer))
						{
							GC.ScreenShake(0.2f, 80f, __instance.myMelee.agent.tr.position, __instance.myMelee.agent);
							GC.FreezeFrames(1);
						}
						GC.alienFX.BlockAttack(__instance.myMelee.agent);
						GC.alienFX.BlockAttack(melee.agent);
						if (!__instance.myMelee.agent.killerRobot && !melee.agent.killerRobot)
						{
							GC.EnforcerAlertAttack(__instance.myMelee.agent, melee.agent, 7.4f);
							GC.EnforcerAlertAttack(melee.agent, __instance.myMelee.agent, 7.4f);
						}
						GC.playerControl.Vibrate(__instance.myMelee.agent.isPlayer, Mathf.Clamp((float)__instance.myMelee.invItem.meleeDamage / 100f + 0.05f, 0f, 0.25f), Mathf.Clamp((float)__instance.myMelee.invItem.meleeDamage / 132f + 0.05f, 0f, 0.2f));
						__instance.myMelee.agent.combat.meleeJustBlockedCooldown = __instance.myMelee.agent.combat.meleeJustBlockedTimeStart;
						...
						if ((__instance.myMelee.agent.statusEffects.hasTrait("ChanceToKnockWeapons") || __instance.myMelee.agent.statusEffects.hasTrait("KnockWeapons")) && GC.serverPlayer && agent4.isPlayer == 0 && agent4.inventory.equippedWeapon.invItemName != "Fist" && !agent4.warZoneAgent)
						{
							int myChance4 = 15;
							bool flag7 = __instance.myMelee.agent.statusEffects.hasTrait("KnockWeapons");
							if (!flag7)
							{
								myChance4 = __instance.myMelee.agent.DetermineLuck(15, "ChanceToKnockWeapons", true);
							}
							if (GC.percentChance(myChance4) || flag7)
							{
								...
								if (!GC.serverPlayer)
								{
									agent4.objectMultPlayfield.SpawnStatusText("OutOfAmmo", invItem3.invItemName, "Item", __instance.myMelee.agent.objectNetID, "", "");
								}
								agent4.statusEffects.CreateBuffText("DroppedWeapon");
								agent4.dontPickUpWeapons = true;
							}
						}
						if ((agent4.statusEffects.hasTrait("ChanceToKnockWeapons") || agent4.statusEffects.hasTrait("KnockWeapons")) && GC.serverPlayer && __instance.myMelee.agent.isPlayer == 0 && invItem.invItemName != "Fist" && !agent4.warZoneAgent)
						{
							int myChance5 = 15;
							...
							{
								InvItem invItem4 = __instance.myMelee.agent.inventory.FindItem(invItem.invItemName);
								__instance.myMelee.agent.inventory.DestroyItem(invItem4);
								GC.spawnerMain.SpillItem(__instance.tr.position, invItem4);
								GC.spawnerMain.SpawnStatusText(__instance.myMelee.agent, "OutOfAmmo", invItem4.invItemName, "Item");
								if (!GC.serverPlayer && (__instance.myMelee.agent.isPlayer != 0 || __instance.myMelee.agent.mindControlAgent == GC.playerAgent))
								{
									__instance.myMelee.agent.objectMultPlayfield.SpawnStatusText("OutOfAmmo", invItem4.invItemName, "Item", __instance.myMelee.agent.objectNetID, "", "");
								}
								__instance.myMelee.agent.statusEffects.CreateBuffText("DroppedWeapon", __instance.myMelee.agent.objectNetID);
								__instance.myMelee.agent.dontPickUpWeapons = true;
								return false;
				...
				else if (hitObject.CompareTag("BulletHitbox"))
				{
					...
					if (__instance.myMelee.agent.isPlayer > 0)
					{
						bool localPlayer = __instance.myMelee.agent.localPlayer;
						return false;
					}
				}
				else if (hitObject.CompareTag("Wall"))
				{
					...
					if (!__instance.hitWall && !__instance.myMelee.invItem.meleeNoHit)
					{
						...
						bool flag8 = false;
						int num6 = __instance.myMelee.agent.FindDamage(__instance.myMelee, true, fromClient);
						if (__instance.myMelee.agent.agentSpriteTransform.localScale.x > 1f)
						{
							num6 = 200;
						...
						if (__instance.myMelee.agent.statusEffects.hasTrait("MeleeDestroysWalls2") && invItem.invItemName != "Fist")
						{
							num7 = 0;
							num8 = 0;
							num9 = 0;
						...
						else if (__instance.myMelee.agent.statusEffects.hasTrait("MeleeDestroysWalls") && invItem.invItemName != "Fist")
						{
							num7 = 0;
							num8 = 0;
							num9 = 0;
						...
						if (num6 >= num7)
						{
							...
							{
								Door.freerAgent = __instance.myMelee.agent;
								GC.tileInfo.DestroyWallTileAtPosition(hitObject.transform.position.x, hitObject.transform.position.y, Vector3.zero, true, __instance.myMelee.agent, false, true, false, __instance.myMelee.agent, false);
								GC.audioHandler.Play(__instance.myMelee.agent, "WallDestroy");
								if (hitObject.name.Contains("Glass"))
								{
									GC.audioHandler.Play(__instance.myMelee.agent, "WallDestroyGlass");
								}
								hitObject.layer = 1;
								...
								if (__instance.myMelee.agent.isPlayer > 0 && __instance.myMelee.agent.localPlayer)
								{
									GC.stats.AddToStat(__instance.myMelee.agent, "Destruction", 1);
								}
								GC.ScreenShake(0.25f, 160f, __instance.myMelee.agent.tr.position, __instance.myMelee.agent);
								...
								if (invItem.invItemName != "Fist")
								{
									...
									if (__instance.myMelee.agent.statusEffects.hasTrait("MeleeDestroysWalls2"))
									{
										__instance.myMelee.agent.inventory.DepleteMelee(10);
									}
									else
									{
										__instance.myMelee.agent.inventory.DepleteMelee(20);
									}
								}
							}
						}
						...
						if (!__instance.myMelee.successfullySleepKilled && !__instance.myMelee.agent.statusEffects.hasTrait("HitObjectsNoNoise") && (GC.serverPlayer || (!GC.serverPlayer && (__instance.myMelee.agent.localPlayer || __instance.myMelee.agent.mindControlAgent == GC.playerAgent))))
						{
							GC.spawnerMain.SpawnNoise(__instance.myMelee.agent.tr.position, 1f, null, null, __instance.myMelee.agent);
							if (flag8)
							{
								GC.spawnerMain.SpawnNoise(hitObject.transform.position, 1f, null, null, __instance.myMelee.agent);
							}
						}
						if (flag8 && __instance.myMelee.agent != null)
						{
							GC.OwnCheck(__instance.myMelee.agent, hitObject, "Normal", 0);
						}
						GC.audioHandler.Play(__instance.myMelee.agent, "BulletHitWall");
						GC.playerControl.Vibrate(__instance.myMelee.agent.isPlayer, Mathf.Clamp((float)__instance.myMelee.invItem.meleeDamage / 100f + 0.05f, 0f, 0.25f), Mathf.Clamp((float)__instance.myMelee.invItem.meleeDamage / 132f + 0.05f, 0f, 0.2f));
						if (!GC.serverPlayer && (__instance.myMelee.agent.localPlayer || __instance.myMelee.agent.mindControlAgent == GC.playerAgent))
						{
							if (__instance.myMelee.agent.isPlayer != 0)
							{
								__instance.myMelee.agent.objectMult.CallCmdMeleeHitWall(hitObject.transform.position);
								return false;
							}
							GC.playerAgent.objectMult.CallCmdMeleeHitWallNPC(__instance.myMelee.agent.objectNetID, hitObject.transform.position);
						}
					}
				}
			}
