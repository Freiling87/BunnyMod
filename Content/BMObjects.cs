	...
	public class BMObjects
    {
        ...
        public static void ObjectReal_DetermineButtons(ObjectReal __instance) // Postfix
        {
            ...
            else if (__instance is Manhole)
			{
                ...
                if (manhole.opened && agent.statusEffects.hasTrait(cTraits.UnderdarkCitizen))
        ...
        public static bool ObjectReal_PressedButton(string buttonText, int buttonPrice, ObjectReal __instance) // Replacement
        {
            ...
            if (buttonText == "CollectPart") // Vanilla
            {
                ...
                if (!__instance.interactingAgent.statusEffects.hasTrait(vTraits.SneakyFingers) && __instance.functional)
                {
        ...
        {
            if (BMTraits.IsTraitActive(cTraits.StealthBastardDeluxe))
		...
		public static bool Hole_EnterRange(GameObject myObject, Hole __instance) // Prefix
		{
            if (GC.loadComplete)
                if (myObject.CompareTag("Agent"))
                {
                    ...
                    if (__instance.GetComponent<ObjectMultHole>().objectHoleType == "Manhole" && agent.statusEffects.hasTrait(cTraits.UnderdarkCitizen))
                    {
		...
		public static void Manhole_FlushYourself(Manhole __instance) // Non-Patch
		{
			...
			if ((agent.statusEffects.hasTrait(cTraits.UnderdarkCitizen)) && !agent.statusEffects.hasStatusEffect("Giant"))
			{
		...
		{
            if (myAgent.statusEffects.hasTrait(vTraits.Bulky))
            {
            ...
            if (!myAgent.statusEffects.hasTrait(cTraits.StealthBastardDeluxe))
