...
namespace BunnyMod.Content
{
	public class BMTraits
	{
		...
		public void Awake()
		{
			...
			//BlessedStrikes.IsUpgrade = false;
			...
			//BlessedStrikes_2.IsUpgrade = true;
			...
			CustomTrait Ballistician = RogueLibs.CreateCustomTrait(cTrait.Ballistician, true,
				new CustomNameInfo("Ballistician"),
			...
			CustomTrait Ballistician_2 = RogueLibs.CreateCustomTrait(cTrait.Ballistician_2, true,
				new CustomNameInfo("Ballistician +"),
				new CustomNameInfo(
					"When you're shooting people, you like to yell, \"Pew Pew!\" Who's going to stop you, anyway?\n\nYour bullets fly faster, further & harder"));
			Ballistician_2.Available = false;
			Ballistician_2.AvailableInCharacterCreation = false;
			Ballistician_2.CanRemove = false;
			Ballistician_2.CanSwap = true;
			Ballistician_2.Conflicting.AddRange(new string[] { cTrait.Myopic, cTrait.Myopic2 });
			Ballistician_2.CostInCharacterCreation = 8;
			Ballistician_2.IsActive = true;
			Ballistician_2.Upgrade = null;

			CustomTrait DoubleTapper = RogueLibs.CreateCustomTrait(cTrait.DoubleTapper, true,
				new CustomNameInfo("Double-Tapper"),
				new CustomNameInfo(
					"You're not some kind of monster. A quick bullet to the back of the head is more humane than the alternatives, isn't it?\n\nBullet attacks from behind within melee range do 2x damage. 10x if you're invisible or hidden."));
			...
			CustomTrait Sniper = RogueLibs.CreateCustomTrait(cTrait.Sniper, true,
				...
				new CustomNameInfo(
					"They can run, but they'll only die tired. But you'll be tired too, so it's best if they just die before running.\n\n- Past a certain distance, Revolver hits deal massive damage\n- Minimum range lower for unaware targets\n- Bullet range increased"));
			...
			CustomTrait Sniper_2 = RogueLibs.CreateCustomTrait(cTrait.Sniper_2, true,
				new CustomNameInfo("Sniper +"),
				new CustomNameInfo(
					"Wow, that's a poncho made of garbage, isn't it? Well, whatever it takes to blend into the urban environment.\n\n- Using a silent ranged weapon while hidden behind a Bush or other object will not remove you from cover."));
			...
			CustomTrait DAREdevil = RogueLibs.CreateCustomTrait(cTrait.DAREdevil, true,
				...
				new CustomNameInfo("You have injected zero marijuanas. Crack is Whack. Smokers are Jokers. Needles are for... beetles.", "", "", "", "",
					"За свою жизнь вы не разу не употребили марихуану. Для вас крэк - трещина, курилщики - шутники, а иголки для ежей.", "", ""));
			...
			CustomTrait FriendOfBill = RogueLibs.CreateCustomTrait(cTrait.FriendOfBill, true,
				...
				new CustomNameInfo("You are taking things one day at a time. But every day sucks when you can't get drunk anymore.", "", "", "", "",
					"Вы пьёте одну бутылку за другой, но каждый день без выпивки - это отстой!", "", ""));
			...
			CustomTrait Teetotaller = RogueLibs.CreateCustomTrait(cTrait.Teetotaller, true,
				...
				new CustomNameInfo("Wow, you're really boring. You don't do drugs *or* alcohol. What do you even do?", "", "", "", "",
					"Удивительно но вы действительно скучный. Вы не употребляете наркотики или алкоголь.. Вы вообще живой?", "", ""));
			...
			CustomTrait AfraidOfLoudNoises = RogueLibs.CreateCustomTrait(cTrait.AfraidOfLoudNoises, true,
				...
				new CustomNameInfo(
					"The recoil bruised my shouldah. The brass shell casings disoriented me as they flew past my face. The smell of sulfur and destruction made me sick. The explosions - loud like a bowomb - gave me a temporary case of PTSD. For at least an hour after firing the gun just a few times, I was anxious and irritable. And it's such small portions!",
					"", "", "", "",
					"Отдача от оружия сильно ударяет меня в плечё. Пули пролетая мимо меня сильно дезоринтируют, а от запаха серы у меня проявляются рвотные рефлексы. Громкие взрывы вызывают у меня приступы ПТСР. Всё это я узнал когда выстрелил одну пулю из пистолета.. и это лишь одна пуля..",
					"", ""));
			...
			CustomTrait DrawNoBlood = RogueLibs.CreateCustomTrait(cTrait.DrawNoBlood, true,
				...
				new CustomNameInfo(
					"You have taken an oath to draw no blood. Guess you'll have to smash skulls really carefully, then.\n\n- Can't use bullet-based guns, sharp weapons, or most explosives.",
					"", "", "", "",
					"Вы приняли присягу не проливать ни капли крови.. Думаю вам надо будет крайней тщательно ломать черепа.\n\nВы не можете использовать огнестрельное оружие, колющее и режущее оружие, а также большинство взрывчатых веществ.",
					"", ""));
			...
			CustomTrait FatHead = RogueLibs.CreateCustomTrait(cTrait.FatHead, true,
				...
				new CustomNameInfo(
					"You have a big, fat, ugly head. You can't wear hats of any kind. No one will lend you their headphones or sunglasses, because your big, fat, dumb, ugly head will break them. Your self-esteem is pretty much in the toilet.",
					"", "", "", "",
					"У вас большая, жирная и уродливая голова. Вы даже не можете носить никакие головные уборы. Никто вам не одолжит солнцезащитные очки или наушники, вы их просто сломаете своей тупой, жирной головой. Неудивительно, что ваша самооценка ниже плинтуса.",
					"", ""));
			...
			CustomTrait Brainiac = RogueLibs.CreateCustomTrait(cTrait.Brainiac, true,
				...
				new CustomNameInfo(
					"You have a ton of potential, and pretty much nothing else. Hopefully you live past level 1. We're rooting for you, buddy.\n\n-XP gain set to 400%"));
			...
			CustomTrait MoronTheMerrier = RogueLibs.CreateCustomTrait(cTrait.MoronTheMerrier, true,
				...
				new CustomNameInfo(
					"You are not playing with a full set of brain cells. You are not the sharpest tool in the dumb-person storage shed. The lights are on, but someone is dumb, and it's you. Are you understanding any of this?\n\n- XP gain set to 50%"));
			...
			CustomTrait SmoothBrained = RogueLibs.CreateCustomTrait(cTrait.SmoothBrained, true,
				...
				new CustomNameInfo(
					"You emerged from your mother's womb fully formed, killing her in the process. The terrified doctor declared you a perfect child and in no need of improvement in any way, ever.\n\n- XP gain set to 0%"));
			...
			CustomTrait VeryHardOnYourself = RogueLibs.CreateCustomTrait(cTrait.VeryHardOnYourself, true,
				...
				new CustomNameInfo(
					"You were a rigid pianist for years: Any time you commited a boner or a cock-up, you would ejaculate in frustration. Your life was seemin' saturated, so you thought you'd come to the Resistance. They appreciate your perfectionism - the hardest job for you to swallow is a wrecked one.\n\n- Anytime you miss an XP bonus, suffer a penalty\n- Any negative XP is doubled\n\nPenis"));
			...
			CustomTrait BodyGuarded_2 = RogueLibs.CreateCustomTrait(cTrait.BodyGuarded_2, true,
				new CustomNameInfo("BodyGuarded +", "", "", "", "", "", "", ""),
				new CustomNameInfo(
					"Oooooooooooooooh, look even harder at the bigger more important VVIP here. I bet even more that you feel even fancier. La de da deedly doodly da!\n\n- Two Supergoons join you on each level\n- Doubles size of trait description list"));
			...
			CustomTrait Charmed = RogueLibs.CreateCustomTrait(cTrait.Charmed, true,
				...
				new CustomNameInfo("You once found a fourteen-leaf clover.\n\n- Stacks with Four-Leaf Clover", "", "", "", "",
					"Как-то раз вы нашли 14 листьев клевера.", "", ""));
			...
			CustomTrait Charmed_2 = RogueLibs.CreateCustomTrait(cTrait.Charmed_2, false,
				...
				new CustomNameInfo("You are *really* lucky. Anyone who's been at the toilet next to you can attest.\n\n- Stacks with Four-Leaf Clover", "", "",
					"", "", "Вам действительно повезло. Любой кто был у туалета рядом с вами это подтвердит.", "", ""));
			...
			CustomTrait Cursed = RogueLibs.CreateCustomTrait(cTrait.Cursed, true,
				...
				new CustomNameInfo("You pissed in some old Gypsy lady's cereal, and you still refuse to apologize. She didn't like that.", "", "", "", "",
					"Вы помочились в кашу старой заклинательницы и до сих пор не извинились. Ей это явно не нравится.", "", ""));
			...
			CustomTrait Cursed_2 = RogueLibs.CreateCustomTrait(cTrait.Cursed_2, true,
				new CustomNameInfo("Unluckier", "", "", "", "", "Несчастный +", "", ""),
				new CustomNameInfo(
					"You bought up an old Indian graveyard, and there you built a black cat sanctuary and mirror-breakery. Not your best choice.", "", "", "",
					"", "Вы купили старое индийское кладбище и построили там приют для чёрных кошек. Не самый лучший выбор.", "", ""));
			...
			CustomTrait FocusedCasting = RogueLibs.CreateCustomTrait(cTrait.FocusedCasting, true,
				...
				new CustomNameInfo("You've carefully refined your magic techniques to improve accuracy and reduce the chances of miscasting spells.", "", "",
					"", "", "Вы тщательно отточили свои магические приёмы, теперь вы стреляете точнее, а шанс на неудачный каст стал ниже.", "", ""));
			...
			CustomTrait FocusedCasting_2 = RogueLibs.CreateCustomTrait(cTrait.FocusedCasting_2, true,
				new CustomNameInfo("Focused Casting +", "", "", "", "", "Фокусировка кастов +", "", ""),
				new CustomNameInfo(
					"You've even more carefully refined your techniques even more to improve accuracy and even more reduce the chances of miscasting spells EVEN MORE.",
					"", "", "", "",
					"Вы ещё более тщательно отточили свои магические приёмы, вы стали стрелять ещё точнее и шанс на неудачный каст стал ещё ниже.", "", ""));
			...
			CustomTrait MagicTraining = RogueLibs.CreateCustomTrait(cTrait.MagicTraining, true,
				...
				new CustomNameInfo("Improves your skills with any Magic Special Ability.", "", "", "", "",
					"Улучшает ваши навыки обращения с магией! Спец. способность.", "", ""));
			...
			CustomTrait MagicTraining_2 = RogueLibs.CreateCustomTrait(cTrait.MagicTraining_2, true,
				...
				new CustomNameInfo("Further improves your skills with any Magic Special Ability.", "", "", "", "",
					"Теперь ещё больше улучшает навыки обращения с магией! Спец. способность.", "", ""));
			...
			CustomTrait WildCasting = RogueLibs.CreateCustomTrait(cTrait.WildCasting, true,
				...
				new CustomNameInfo(
					"You don't need all that safety shit. You wanna cast some damn spells! Your spells are more powerful, but you have a greater chance of miscasting them.",
					"", "", "", "",
					"Безопасность только для лохов! Вы хотите кастовать чёртовы мощные заклинания! Ваши заклинания более мощные, но и шанс на неудачный каст заклинания выше.",
					"", ""));
			...
			CustomTrait WildCasting_2 = RogueLibs.CreateCustomTrait(cTrait.WildCasting_2, true,
				...
				new CustomNameInfo("You're addicted to power. Your spells are ridiculously powerful... and risky.", "", "", "", "",
					"Вы зависимы от магии. Ваши заклинания рискованны, но при этом мощные и смешные.", "", ""));
			...
			CustomTrait RATS = RogueLibs.CreateCustomTrait(cTrait.RATS, true,
				...
				new CustomNameInfo(
					"Resistance-Tec Assisted Targeting System. The latest cybernetic enhancement to shooting accuracy, crit chance, & some combat traits with a double effect when time is slowed down.",
					"", "", "", "",
					"Сопротивленческая Система Помощи Прицеливания. Совершенная кибернетеческая система улучшающая стрельбу, шанс крита, так же некоторые боевые особенности удваиваются, когда замедляется время.",
					"", ""));
			RATS.Available = true;
			...
			RATS.IsActive = true;
			RATS.Recommendations.AddRange(new string[]
				{ vTrait.Butterfingerer, cSpecialAbility.ChronomanticDilation, vTrait.IncreasedCritChance, vTrait.Kneecapper, vTrait.UnCrits });
			...
			CustomTrait RATS_2 = RogueLibs.CreateCustomTrait(cTrait.RATS_2, true,
				...
				new CustomNameInfo(
					"\"Various bug fixes and performance improvements.\" That's all the fucking update notes say. Also, they took out the headphone jack??", "",
					"", "", "",
					"\"Различные исправления багов и улучшение производительности.\" Это всё что сказано про это чёртово обновление! Они ещё и удалили разьём под наушники?!",
					"", ""));
			RATS_2.Available = false;
			...
			RATS_2.Recommendations.AddRange(new string[]
				{ vTrait.Butterfingerer, cSpecialAbility.ChronomanticDilation, vTrait.IncreasedCritChance, vTrait.Kneecapper, vTrait.UnCrits });
			RATS_2.Upgrade = null;
			...
			CustomTrait EagleEyes = RogueLibs.CreateCustomTrait(cTrait.EagleEyes, true,
				...
				new CustomNameInfo("You can see further than normal. Hell, you can see further than *abnormal*.", "", "", "", "",
					"Вы можете видеть дальше, чем обычно. Чёрт! Это же ненормально!", "", ""));
			...
			CustomTrait EagleEyes_2 = RogueLibs.CreateCustomTrait(cTrait.EagleEyes_2, true,
				new CustomNameInfo("Eagle Eyes +", "", "", "", "", "Орлиные глаза +", "", ""),
				new CustomNameInfo(
					"You might have been a good sniper, but you were kicked out of training when they didn't believe that you worked better without a rifle scope.",
					"", "", "", "",
					"Возможно вы были прекрасным снайпером, но вас выгнали на одной из тренировок, видимо они не поверили что вы стреляете лучше без прицела.",
					"", ""));
			...
			CustomTrait Myopic_2 = RogueLibs.CreateCustomTrait(cTrait.Myopic2, true,
				...
				new CustomNameInfo("You tend to keep people at arm's length, where you can't see them.", "", "", "", "",
					"Обычно вы держите людей на расстоянии вытянотой руки, жаль что вы их всё равно не видите.", "", ""));
			...
			BootLicker.Conflicting.AddRange(new string[]
			{
				vTrait.Charismatic, vTrait.CorruptionCosts, vTrait.Wanted, cTrait.Priors, cTrait.GenerallyUnpleasant, cTrait.ObjectivelyUnpleasant,
				vTrait.TheLaw
			});
			...
			CustomTrait Domineering = RogueLibs.CreateCustomTrait(cTrait.Domineering, true,
				...
				new CustomNameInfo(
					"There's just something about how you carry yourself. Maybe it's the way you walk, or maybe it's the way you demand obedience from the weak around you. People will occasionally be Submissive to you. Kinky!",
					"", "", "", "",
					"Есть что-то в том, как ты себя ведешь. Может быть, что дело в том как вы ходите, а может в том, как вы требуете подчинения от слабых вокруг. Люди иногда будут повиноватся вам, но вы крайне эксцентричны!",
					"", ""));
			...
			CustomTrait Domineering_2 = RogueLibs.CreateCustomTrait(cTrait.Domineering_2, true,
				new CustomNameInfo("Domineering +", "", "", "", "", "Доминирующий + ", "", ""),
				new CustomNameInfo(
					"Some people make sure their social skills merely work for them, but you crack the whip! You're finding more and more Subs everywhere you look.",
					"", "", "", "",
					"Некоротые люди убеждаются, что ваши социальные навыки работают на них. Ты треснешь кнутом. Вы находите все больше и больше подводных камней куда бы вы ни посмотрели.",
					"", ""));
			Domineering_2.Available = false;
			...
			CustomTrait GenerallyUnpleasant = RogueLibs.CreateCustomTrait(cTrait.GenerallyUnpleasant, true,
				...
				new CustomNameInfo("You have a certain way with people! A very annoying way.", "", "", "", "",
					"Вы крайне раздражительны, вы даже не умеете общатся с людьми! Это оооочень... раздражает.", "", ""));
			...
			GenerallyUnpleasant.Conflicting.AddRange(new string[]
			{
				vTrait.Antisocial, vTrait.Charismatic, vTrait.FairGame, vTrait.FriendoftheCommonFolk, cTrait.ObjectivelyUnpleasant, vTrait.Malodorous,
				cTrait.Polarizing, cTrait.Polarizing_2, vTrait.Suspicious, vTrait.Wanted
			});
			...
			CustomTrait ObjectivelyUnpleasant = RogueLibs.CreateCustomTrait(cTrait.ObjectivelyUnpleasant, true,
				...
				new CustomNameInfo(
					"You chew with your mouth open. You rightfully have no friends in the world. You are scum. Everyone starts out Annoyed, including me.", "",
					"", "", "",
					"Вы чавкаете во время еды. У вас в этом мире нету ни единого друга. Вы подонок. Все кто с вами встречаются раздражены, включая меня.", "",
					""));
			...
			ObjectivelyUnpleasant.Conflicting.AddRange(new string[]
			{
				vTrait.Antisocial, vTrait.Charismatic, vTrait.FairGame, vTrait.FriendoftheCommonFolk, cTrait.GenerallyUnpleasant, vTrait.Malodorous,
				cTrait.Polarizing, cTrait.Polarizing_2, vTrait.Suspicious, vTrait.Wanted
			});
			...
			CustomTrait Polarizing = RogueLibs.CreateCustomTrait(cTrait.Polarizing, true,
				...
				new CustomNameInfo("Everyone has an opinion on you, when they first meet you. Might be good or bad, but at least you feel noticed!", "", "", "",
					"", "Каждый раз когда с вами встречается новый человек у него всегда будет о вас мнение, хорошее или плохое, зато вас заметили!", "", ""));
			Polarizing.Available = true;
			Polarizing.AvailableInCharacterCreation = false;
			Polarizing.CanRemove = false;
			Polarizing.CanSwap = true;
			Polarizing.Conflicting.AddRange(new string[]
			{
				vTrait.Antisocial, vTrait.Charismatic, vTrait.FairGame, vTrait.FriendoftheCommonFolk, cTrait.GenerallyUnpleasant, cTrait.ObjectivelyUnpleasant,
				vTrait.Malodorous, cTrait.Polarizing_2, vTrait.Suspicious, vTrait.Wanted
			});
			...
			CustomTrait Polarizing_2 = RogueLibs.CreateCustomTrait(cTrait.Polarizing_2, true,
				...
				new CustomNameInfo("People have *strong* opinions of you. Like me. I think you're just great.", "", "", "", "",
					"Люди хорошего мнения о вас. Даже я. Я думаю ты просто чудо.", "", ""));
			Polarizing_2.Available = false;
			Polarizing_2.AvailableInCharacterCreation = true;
			Polarizing_2.CanRemove = false;
			Polarizing_2.CanSwap = true;
			Polarizing_2.Conflicting.AddRange(new string[]
			{
				vTrait.Antisocial, vTrait.Charismatic, vTrait.FairGame, vTrait.FriendoftheCommonFolk, cTrait.GenerallyUnpleasant, cTrait.ObjectivelyUnpleasant,
				vTrait.Malodorous, cTrait.Polarizing, vTrait.Suspicious, vTrait.Wanted
			});
			...
			CustomTrait Priors = RogueLibs.CreateCustomTrait(cTrait.Priors, true,
				...
				new CustomNameInfo("Your ink marks you as a scourge to law enforcers. On the upside, you have the respect of the City's criminals.", "", "", "",
					"",
					"У вас длинный список преступлений и каждый коп знает вас в лицо. Они так устали от вас и вашего дерьма, что только и ищут повод, чтобы избить вас.",
					"", ""));
			...
			CustomTrait VeiledThreats = RogueLibs.CreateCustomTrait(cTrait.VeiledThreats, true,
				...
				new CustomNameInfo("When you attempt to Bribe, Extort, Mug, or Threaten, a failure will turn the target Annoyed instead of Hostile.", "", "",
					"", "", "Когда вы пытаетесь угрожать, подкупить, грабить если ваша цель откажется, то она будет раздражена, а не враждебна.", "", ""));
			...
			CustomTrait MookMasher = RogueLibs.CreateCustomTrait(cTrait.MookMasher, true,
				new CustomNameInfo("Mook Masher", "", "", "", "", "Разрушитель сердец", "", ""),
				new CustomNameInfo(
					"The Mayor knows you're a threat, and you're coming for him. He could just destroy you, but as a villain, he prefers to send his henchmen at you in steadily increasing but manageable waves.",
					"", "", "", "",
					"Когда вы пришли к Мэру, он знал что вы угроза. Он мог бы просто вас убить, но как любой злодей, он предпочтёл отправляться волны приспешников, волны постоянно растут, но они управляемые.",
					"", ""));
			...
			CustomTrait Reinforcements = RogueLibs.CreateCustomTrait(cTrait.Reinforcements, true,
				...
				new CustomNameInfo(
					"You have worked to create an army for the Resistance. That army now patrols the City secretly, looking for the opportunity to aid the cause.",
					"", "", "", "",
					"Вы работали над созданием армии для Сопротивления. На данный момент эта армия тайно патрулирует город, постоянно ищя возможность, чтобы помочь вашему общему делу.",
					"", ""));
			...
			CustomTrait UnderdarkCitizen = RogueLibs.CreateCustomTrait(cTrait.UnderdarkCitizen, true,
				...
				new CustomNameInfo("You can navigate the city's sewers with ease. Their denizens no longer consider you an easy mark.", "", "", "", "",
					"Вы с лёгкостью оринтируетесь в канализации. Жители этих мест не считают вас лёгкой целью.", "", ""));
			...
			CustomTrait TamperTantrum_2 = RogueLibs.CreateCustomTrait(cTrait.TamperTantrum_2, false,
				...
				new CustomNameInfo("Your tools take zero wear when used in tampering.", "", "", "", "",
					"Ваши инструменты не изнашиваются когда вы подделываете что-то.", "", ""));
		...
		public static bool ScrollingMenu_GetTraitsRemoveTrait(ScrollingMenu __instance) // Replacement
		{
			...
			for (int i = 0; i < __instance.agent.statusEffects.TraitList.Count; i++)
			{
				...
				for (int j = 0; j < __instance.gc.sessionDataBig.unlocks.Count; j++)
				{
					...
					if (unlock.unlockName == trait.traitName && unlock.unlockType == "Trait" && unlock.cost3 < 0 &&
						(!unlock.cantLose || __instance.gc.challenges.Contains("NoLimits")))
					{
