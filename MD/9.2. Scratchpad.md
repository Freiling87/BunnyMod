...
[F] = Fixing
[B] = Balancing
[M] = Multiple (see subcategories)

## Unsorted
- Conflict showing Studious twice instead of superstudious
  - Attempted
- Need a custom method that will add trait conflicts while excluding self.
## Mutators
### Police State [M]

- Cameras [F]
  - Police State Cameras don't spawn correctly on North-facing walls.
    - Check out ObjectReal.shiftAmountN, etc.
    - In SecurityCam.SetVars:
    ``` 
		this.shiftAmountEW = 0.2f;
		this.shiftAmountN = 0.36f;
    ``` 
    - In ObjectReal.ShiftTowardWalls:
    ```
    if (this.shiftAmountN == 99f)
		this.shiftAmountN = 0.24f;
	if (this.shiftAmountS == 99f)
		this.shiftAmountS = -0.16f;
	if (this.shiftAmountEW == 99f)
		this.shiftAmountEW = 0.12f;
    ```
    - So we have candidate values to Postfix SetVars with.
    - It is possible that SecurityCam has Snap-To-Wall behaviors built-in that Trashcans don't. You might not need to add ShiftAmount values, but rather remove this algorithm's snap-to-wall portions in order to not interrupt the Camera's code from doing its thing.
    - Try -0.16 first. 
...
- Remember, the overhaul mutators are *out of scope*. Once you fix this, let it be until the next release.
### Spelunky Dory [M]
  - Add random walls to outside of walls, use Trashcan algorithm [I]
    - Attempted
### Sunken City & Transit Experiment [I]
    ```
...
- Cops go hostile when opened, even if not in view. 
  - Make annoyed instead, interrupt and warn if possible
  - Vision check
- Water splash
- Manhole to Toilet
- Toilet to Manhole
...
Baseline: Backstab a thief with sledgehammer for 10xp
  - Dim Bulb
    - Attempted
      - 37xp
  - Moron The Merrier
...
### Underdark Citizen
  - Add Water splash the first time you emerge from each manhole
  - Check if only Manhole open: if not, do not emerge from this one.
### Warlord
  - No effect
...
These are out of scope for this update, but they're halfway done so I might as well continue them for the next one.
...
This should be a list of the features for the current update designated to be within scope. Ideally, finish this before you even begin modding.
