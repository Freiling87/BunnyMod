...
- This:
        - [Message: Bunny Mod] SetGangCount
            [Info   : Unity Log] Window Lose Focus
            [Info   : Unity Log] Loading Mafia
            [Info   : Unity Log] Loading Object Agents
            [Info   : Unity Log] Loading Recycled Agents: 10
...
- BMAbilities
  - DW
  - I don't think these are called for the most part.
- BMAgents
- BMBehaviors
- BMChallenges
- BMCombat
...
## Disco City Danceoff
- Turntables
  - Works, but always busted by default
- Speakers
  - Works, but needs Direction
- Jukeboxes
  - Attempted
- Lights
  - Only worked on home base
## Eagle Eyes
...
- Things to try:
  ...
  - Agent.AgentOnCamera()
    - Attempted
      - If this doesn't work, try multiplying rather than dividing by GetZoomLevel()
...
## Floor Mod Exteriors 
- Interior Floors were somewhere turned into grass, I think Rug type. 
...
- BasicSpawn
  - Spawn - This one calls BasicFloor.Spawn
- LoadLevel
  - FillFloors
  - FillMapChunks
  - LoadStuff2
  ...
  - Defaulting to Slums, check if that's the bad-input default in case water and ice can't be passed. 
    - It might also be that visuals for those tile groups will have to be set separately from their actual floor behaviors.
- ObjectMult
...
- Spawn Speakers, Turntables & Jukeboxes throughout city
  - All operable as normal, plus special uses (not sure yet)
