...
-
## Knockback mods
- WWW doesn't seem to work. I think there's a cap downstream of this method on how much this can affect it.
## Slot Machine
- Jackpot
  - Seems final but didn't get to *really* test it.
  - Maybe split Jackpot into a normal natural version, and a hacked version.
    - The hacked version is the one that has crazy volume and attracts slum dwellers to start a riot. 
    - The vanilla version just plays the song and drops the money.
## Sniper
- Restrict to Revolver
  - Patch SpawnBullet with a Dict<Bullet, Weapon> to track what weapon type it came from
  - Delete them when bullets are deleted 
  - Already have an attempt in, see if that works first
    - Attempted
...
- BasicFloors.Spawn - Interior Floors
  - This works fine! Now detect edge cases that you never want to replace. Graveyards are turning into the chosen type.
  - SpelunkyDory
    - Consider lakes, if you get exterior floors working.
  - CityOfSteel
    - Factory Ambience default?
  - Panoptikopolis
    - Graveyard Ambience default? See the list, who knows.
  - Green Living
    - Good to go!
  - Shanty Town
    - Doubt there's any good ambience for this one, I guess neutral is good.
- Try RandomFloorsWalls in the method we used for walls.
## Spectral Strikes
- Works!
- Not playing any audio though, may need to restore some of that A/V method you gutted
- Add Undead damage
- Add requirement of lvl2 for Weapons (maybe restrict to sword only, to make it special-er)
- Consider parallel traits:
  - Hands of Fire
    - Unarmed & Bat
    - Cause fire
    - Cooks all your Fud
...
## Stove
- Check that it's creating the right number of Cooked Fud, it seems inconsistent
## Tampering (all)
- Ensure Sneaky Fingers applies
- These are playing noise/animations where they didn't use to. Check vanilla and see where it shouldn't be.
...
- Quantity is the amount you get in a custom character though, not a reward amount. Find the generation list for the Vendor Stand drop and see
  - I'm actually going to accept this, since it will reward keeping the cart intact.
